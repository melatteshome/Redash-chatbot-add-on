...
def traverse_directory(connection,directory):
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            folder_name = re.sub(r'\([^)]*\)', '', os.path.basename(root)).replace(' ', '').replace('_', '').replace('-', '_').lower()
            table_name = folder_name + '_' + re.sub(r'\([^)]*\)', '', file.split('.')[0]).replace(' ', '').replace('_', '').replace('-', '_').lower()
            headers = [re.sub(r'\([^)]*\)', '', header).replace(' ', '').replace('_', '').replace('-', '_').lower() for header in get_csv_headers(file_path)]
            try:
                # print (headers)
                # print (table_name)
                # print (folder_name)
...
def get_csv_headers(csv_file_path):
    with open(csv_file_path, 'r') as file:
        ...
        headers = next(reader)
    return headers
def create_table(connection_params, table_name, headers):
    connection = psycopg2.connect(**connection_params)
...
def insert_data(connection_params, table_name, csv_file_path):
    connection = psycopg2.connect(**connection_params)
